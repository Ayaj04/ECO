...
/** The button drops out from under the navbar this long after the page loads. */
const READY_AFTER_MS = 900;
/** Once, shortly after that, it opens to show its name, so the phone is not a mystery on touch screens. */
...
interface CallButtonProps {
  ...
  onOpen: () => void;
  /** Fades the button away, for example while a navbar dropdown is open over it. */
  dimmed?: boolean;
}

/**
 * A ringing phone under the navbar. At rest it is a white coin in a red ring, with soft ripples
 * and a phone that shakes now and then. On hover or focus the ring stretches out to spell "Contact Us".
 */
export default function CallButton({ onOpen, dimmed = false }: CallButtonProps) {
  ...
  return (
    <motion.div
      ...
      animate={shown ? { opacity: 1, y: 0, scale: 1 } : { opacity: 0, y: -8, scale: 0.85 }}
    ...
    >
      ...
      >
        ...
        <span className="relative z-10 grid size-11 shrink-0 place-items-center rounded-full bg-white shadow-[inset_0_-2px_6px_rgba(0,0,0,0.08)] md:size-12">
          <span aria-hidden="true" className="call-emoji text-[20px] leading-none md:text-[22px]">
            📞
          </span>
        </span>
      </button>
