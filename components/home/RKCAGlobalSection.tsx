...
const MAP = {
  width: 1774,
  height: 887,
  hub: { x: 65.44, y: 47.46, label: "India (Mumbai)" },
  // Each dot's x/y must exactly match the arc endpoint in mapArcs.ts (px / map-dimension * 100)
  dots: [
    { key: "australia",    label: "Australia",       x: 84.50, y: 81.85 },
    { key: "japan",        label: "Japan",            x: 82.36, y: 41.04 },
    { key: "taiwan",       label: "Taiwan",           x: 79.48, y: 51.18 },
    { key: "thailand",     label: "Thailand",         x: 73.79, y: 56.26 },
    { key: "china",        label: "China",            x: 73.17, y: 42.73 },
    { key: "qatar",        label: "Qatar",            x: 56.82, y: 48.37 },
    { key: "egypt",        label: "Egypt",            x: 49.49, y: 46.79 },
    { key: "morocco",      label: "Morocco",          x: 36.47, y: 45.21 },
    { key: "south_africa", label: "South Africa",     x: 46.79, y: 78.80 },
    { key: "italy",        label: "Italy",            x: 44.48, y: 42.39 },
    { key: "france",       label: "France",           x: 40.76, y: 37.66 },
    { key: "germany",      label: "Germany",          x: 43.97, y: 32.69 },
    { key: "hungary",      label: "Hungary",          x: 46.90, y: 35.96 },
    { key: "denmark",      label: "Denmark",          x: 43.80, y: 27.85 },
    { key: "uk",           label: "United Kingdom",   x: 40.70, y: 31.00 },
    { key: "ireland",      label: "Ireland",          x: 38.84, y: 31.12 },
    { key: "norway",       label: "Norway",           x: 44.59, y: 23.11 },
    { key: "sweden",       label: "Sweden",           x: 45.77, y: 24.80 },
    { key: "canada",       label: "Canada",           x: 20.41, y: 30.67 },
    { key: "brazil",       label: "Brazil",           x: 21.65, y: 71.03 },
  ],
} as const;
      ...
      >
        ...
        >
          ...
          >
            ...
            {/* City points — positioned to exactly match arc endpoints */}
            {MAP.dots.map((d, n) => {
              const isHovered = hoveredCountry === d.key;
              return (
                <motion.span
                  key={d.key}
                  className="absolute group/pin"
                  style={{ left: `${d.x}%`, top: `${d.y}%`, opacity: dotOpacity, zIndex: isHovered ? 20 : 10 }}
                  onMouseEnter={() => setHoveredCountry(d.key)}
                  onMouseLeave={() => setHoveredCountry(null)}
                >
                  {/* Ping ring */}
                  <span
                    className="rkca-dot-ping pointer-events-none absolute left-0 top-0 rounded-full border border-ecovis-red/70"
                    style={{
                  ...
                  {/* Core dot */}
                  <span
                    className="pointer-events-none absolute left-0 top-0 -translate-x-1/2 -translate-y-1/2 rounded-full transition-transform duration-200"
                    style={{
                      width: isHovered ? "clamp(9px, 0.75vw, 14px)" : "clamp(6px, 0.5vw, 10px)",
                      aspectRatio: "1",
                      background: "#f0281c",
                      boxShadow: isHovered
                        ? "0 0 0 2px rgba(255,255,255,0.98), 0 0 18px 6px rgba(225,6,0,0.7)"
                        : "0 0 0 1.5px rgba(255,255,255,0.95), 0 0 12px 3px rgba(225,6,0,0.5)",
                    }}
                  />
                  {/* Country label tooltip */}
                  {isHovered && (
                    <span
                      className="pointer-events-none absolute left-1/2 -translate-x-1/2 whitespace-nowrap rounded-md bg-white/95 px-2 py-0.5 font-heading text-[10px] font-bold tracking-wide text-ecovis-black shadow-lg backdrop-blur-sm"
                      style={{ bottom: "calc(100% + 8px)", border: "1px solid rgba(225,6,0,0.2)" }}
                    >
                      {d.label}
                    </span>
                  )}
